...
function renderTopCategoryBar(parentOverride){
 ...
 const selected = cat?.value || '';
 const children = childrenOf(parent).filter(c=>!isBlockedCategory(c));
 const buttons = [
...
async function renderCatalogMenu(){
  ...
  $$('.mega-parent').forEach(b=>{b.onmouseenter=()=>{$$('.mega-parent').forEach(x=>x.classList.remove('active'));b.classList.add('active');const p=parents.find(x=>catId(x)===b.dataset.parent);if(p)render(p)};b.onclick=()=>{const p=parents.find(x=>catId(x)===b.dataset.parent);if(p)window.location.href='catalog.html?category='+encodeURIComponent(catName(p))};});
}
...
async function load(){await waitUserCartReady();cart=getCurrentUserCart();updateCart();items=await getCollection(COLLECTIONS.products);try{categories=await getCollection(COLLECTIONS.categories)}catch(e){}categories=categories.filter(c=>!isBlockedCategory(c));categories.sort((a,b)=>Number(a.order??999)-Number(b.order??999)||String(a.title||a.name||'').localeCompare(String(b.title||b.name||''),'ru'));const opts=new Map();categories.forEach(c=>opts.set((c.title||c.name||'').toLowerCase(),c.title||c.name));items.forEach(p=>{const g=group(p);if(g&&g!=='Без группы'&&!isBlockedCatalogName(g))opts.set(g.toLowerCase(),g)});cat&&(cat.innerHTML='<option value="">Все группы</option>'+[...opts.values()].map(x=>`<option value="${x}">${x}</option>`).join(''));await renderCatalogMenu();const params=new URLSearchParams(location.search);if(params.get('category'))cat.value=params.get('category');if(params.get('search')){search.value=params.get('search');topSearch&&(topSearch.value=params.get('search'))}renderCategoryFilter();renderTopCategoryBar();setupCatalogTopControls();setupCatalogDragScroll();updateCart();render()}
function card(p){const d=discount(p),op=oldPrice(p),priceNum=Number(p.price||0),s=stock(p),href=`product.html?id=${p.id}`;return `<article class="as-desktop-product-card catalog-card" data-product-href="${href}"><div class="catalog-card-photo product-img as-card-photo"><a class="product-image-link" href="${href}">${d?`<span class="discount-badge">-${d}%</span>`:''}${image(p)?`<img loading="lazy" decoding="async" src="${image(p)}" alt="${title(p)}">`:'<span>Фото</span>'}</a></div><button class="fav-btn ${favs.includes(p.id)?'active':''}" data-fav="${p.id}" type="button" aria-label="Избранное">${favs.includes(p.id)?'♥':'♡'}</button><a class="catalog-card-link product-card-link" href="${href}"><div class="catalog-card-body as-card-info"><h3 class="catalog-card-title product-title">${title(p)}</h3><div class="catalog-card-category product-group">${group(p)}</div></div><div class="catalog-card-price-area as-card-bottom"><div class="price-row-card"><div class="catalog-card-price price-current">${money(p.price)}</div>${op?`<div class="old-price price-old">${money(op)}</div>`:''}</div><div class="catalog-card-stock as-card-stock">${s>0?'В наличии':'Нет в наличии'}</div></div></a><button class="catalog-cart-btn as-card-cart" data-cart="${p.id}" type="button" ${s<=0?'disabled':''} aria-label="В корзину">🛒</button></article>`}
