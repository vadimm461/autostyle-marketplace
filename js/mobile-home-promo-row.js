...
async function renderPromo(){
  ...
  mount.innerHTML = `${horizontalMarkup ? `<section class="m-section m-promo-group m-promo-group-horizontal"><div class="m-section-head m-promo-section-head"><h2>Акции и подборки</h2></div><div class="m-promo-row m-promo-row-horizontal">${horizontalMarkup}</div></section>` : ''}${verticalMarkup ? `<section class="m-section m-promo-group m-promo-group-vertical"><div class="m-section-head m-promo-section-head"><h2>Спецпредложения</h2></div><div class="m-promo-row m-promo-row-vertical${verticalRowClass}">${verticalMarkup}</div></section>` : ''}`;
