...
function buildCategoryOptions(cats) {
  ...
  const parents = sorted.filter(c => !c.parentId);
  const children = sorted.filter(c => c.parentId);

  return parents.map(parent => {
    const parentTitle = parent.title || parent.name || 'Без названия';

    const childOptions = children
      .filter(child => child.parentId === parent.id)
      .map(child => {
        const childTitle = child.title || child.name || 'Без названия';
        return `<option value="${childTitle}">${Number(child.order ?? 0)} — ${parentTitle} / ${childTitle}</option>`;
      })
      .join('');

    return `
      <option value="${parentTitle}">${Number(parent.order ?? 0)} — ${parentTitle}</option>
      ${childOptions}
    `;
  }).join('');
}
