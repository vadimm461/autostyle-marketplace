...
function card(p){
  ...
  return `<article class="catalog-card favorite-card" data-product-href="product.html?id=${encodeURIComponent(p.id)}">
    ...
    <a class="catalog-card-link" href="product.html?id=${encodeURIComponent(p.id)}">
      ...
      <div class="catalog-card-body"><h3>${title(p)}</h3><div class="catalog-card-category">${group(p)}</div><div class="catalog-card-price-area"><div class="price-row-card"><div class="catalog-card-price">${money(p.price)}</div>${op?`<div class="old-price">${money(op)}</div>`:''}</div><div class="installment-badge catalog-installment-badge"></div></div></div>
    </a>
    <button class="catalog-cart-btn${s<=0?' is-unavailable':''}" data-cart="${p.id}" type="button" ${s<=0?'disabled aria-disabled="true"':''}>В корзину</button>
  </article>`;
}
function bind(){
