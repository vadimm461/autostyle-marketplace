...
function renderAuthModal(){
  ...
  modal.innerHTML=`<div class="modal-card auth-clean">
    ...
    <p class="muted auth-modal-sub">Вход и регистрация теперь только по e-mail и паролю.</p>
    <div class="auth-choice-grid auth-choice-grid-two">
      <button class="auth-choice active" data-auth-mode="login-email" type="button">✉️ Вход по почте</button>
    ...
    <form id="registerForm" class="auth-mode-panel" data-auth-panel="register">
      <div class="auth-form-grid">
        <label class="field field-wide">Имя<input id="regName" autocomplete="name" required></label>
      ...
      <p class="muted" style="margin-top:10px">После регистрации придёт письмо подтверждения на почту.</p>
    </form>
...
function bindAuth(){
  ...
  $('#loginForm')?.addEventListener('submit', async e=>{e.preventDefault();try{say('Входим...');await loginEmail($('#loginEmail').value.trim(),$('#loginPass').value);hideModal();location.href='profile.html'}catch(err){say('Ошибка входа: '+(err.message||err),false)}});
  $('#registerForm')?.addEventListener('submit', async e=>{e.preventDefault();try{const p1=$('#regPass').value,p2=$('#regPass2').value;if(p1!==p2){say('Пароли не совпадают.',false);return;}say('Создаём аккаунт...');await registerEmail($('#regName').value.trim(),$('#regEmail').value.trim(),p1);say('Аккаунт создан. Проверьте письмо подтверждения на почте.');}catch(err){say('Ошибка регистрации: '+(err.message||err),false)}});
}
bindAuth();
