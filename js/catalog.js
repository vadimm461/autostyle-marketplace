...
function setupCatalogSidebarControls(){
  const sidebar = document.querySelector('.filters.catalog-filters');
  const tree = document.getElementById('categoryTreeFilter');
  if(!sidebar || !tree) return;

  let controls = document.getElementById('catalogSidebarControls');
  if(!controls){
    controls = document.createElement('div');
    controls.id = 'catalogSidebarControls';
    controls.className = 'catalog-sidebar-controls';
  ...
  const priceBox = document.querySelector('.catalog-top-price');

  if(sortField){
    sortField.classList.add('sidebar-sort-field');
    controls.appendChild(sortField);
  }
  if(zeroBtn){
    zeroBtn.classList.add('sidebar-zero-btn');
    controls.appendChild(zeroBtn);
  }
  if(priceBox){
    priceBox.classList.add('sidebar-price-filter');
    controls.appendChild(priceBox);
  }
}
