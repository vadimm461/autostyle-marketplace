...
function card(p){
  ...
  return `<article class="catalog-card as-wb-card" data-product-href="${href}">
    ...
    <a class="as-wb-info" href="${href}">
      <div class="catalog-card-body as-wb-body">
        <h3 class="as-wb-title">${title(p)}</h3>
        <div class="catalog-card-category as-wb-group">${group(p)}</div>
        <div class="catalog-card-price-area as-wb-price-area"><div class="price-row-card as-wb-price-row"><div class="catalog-card-price as-wb-price">${money(p.price)}</div>${op?`<div class="old-price as-wb-old">${money(op)}</div>`:''}</div></div>
      </div>
    </a>
  </article>`;
}
