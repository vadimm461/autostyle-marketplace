...
function renderTopCategoryBar(){
 ...
 const roots=categories.filter(c=>!(c.parentId||c.parent||c.parentExternalId));
 bar.innerHTML=roots.slice(0,20).map(c=>{
 const n=(c.title||c.name||'').trim();
 return `<button class="top-cat-btn" data-cat="${n}">${n}</button>`;
 }).join('');
 bar.onclick=e=>{
   const btn=e.target.closest('.top-cat-btn');
   if(!btn)return;
   if(cat){cat.value=btn.dataset.cat; render();}
 };
}
...
function money(v){return `${fmt.format(Number(v||0))} ₽`}function stock(p){return Number(p.stock??p.quantity??p.count??1)}function title(p){return p.title||p.name||'Товар'}function image(p){return p.image||p.imageUrl||p.photo||''}function group(p){return p.group||p.category||p.categoryName||'Без группы'}function rawOldPrice(p){return Number(p.oldPrice||p.priceOld||p.compareAtPrice||0)}function oldPrice(p){const op=rawOldPrice(p),pr=Number(p.price||0);return op>pr?op:0}function discount(p){const manual=Number(p.discount||p.discountPercent||0),rawOp=rawOldPrice(p),op=oldPrice(p),pr=Number(p.price||0);if(manual>0)return manual;return op>pr&&pr>0?Math.round((op-pr)/op*100):0}function cartQty(){return (Array.isArray(cart)?cart:[]).reduce((s,i)=>s+(typeof i==='object'?Math.max(1,Number(i.qty??i.quantity??i.count??1)||1):1),0)}function updateCart(){localStorage.setItem('cart',JSON.stringify(cart));const n=cartQty();document.querySelectorAll('#cartCount,.cartCount').forEach(el=>el.textContent=String(n));window.dispatchEvent(new Event('autostyle-cart-updated'))}function saveFav(){localStorage.setItem('favorites',JSON.stringify(favs))}
