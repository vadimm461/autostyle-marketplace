...
function setupCatalogTopControls(){
  ...
  // Безопасное реальное перемещение DOM. Если zeroBtn не прямой ребёнок top,
  // insertBefore ломал загрузку каталога и оставлял "Загрузка товаров...".
  if (zeroBtn.parentNode === top) top.insertBefore(sortField, zeroBtn);
  else top.appendChild(sortField);
}
...
function bind(){$$('[data-cart]').forEach(b=>b.onclick=async e=>{e.preventDefault();try{await addUserCartItem(b.dataset.cart);cart=getCurrentUserCart();updateCart();b.textContent='Добавлено';setTimeout(()=>b.textContent='🛒',900)}catch(err){alert(err?.message||'Войдите в аккаунт, чтобы добавить товар в корзину')}});$$('[data-fav]').forEach(b=>b.onclick=e=>{e.preventDefault();e.stopPropagation();const id=b.dataset.fav;favs=favs.includes(id)?favs.filter(x=>x!==id):[...favs,id];b.classList.toggle('active',favs.includes(id));saveFav()})}
...
if (grid) {
  ...
  cat?.addEventListener('change',()=>{renderCategoryFilter();render();});
  sort?.addEventListener('change',render);
  $('#priceFrom')?.addEventListener('input',render);
  $('#priceTo')?.addEventListener('input',render);
  $('#zeroNotice')&&($('#zeroNotice').onclick=()=>{showZero=!showZero;render()});
  setupAuth();
  setupSearch();
  load().finally(()=>window.AutoStyleLoader&&window.AutoStyleLoader.hide());
}
