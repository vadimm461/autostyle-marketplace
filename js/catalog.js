...
function bind(){$$('[data-cart]').forEach(b=>b.onclick=async e=>{e.preventDefault();const product=items.find(p=>String(p.id)===String(b.dataset.cart))||{id:b.dataset.cart};const saved=await addToUserCart(product);if(saved){cart=saved;b.textContent='Добавлено';setTimeout(()=>b.textContent='В корзину',900)}});$$('[data-fav]').forEach(b=>b.onclick=e=>{e.preventDefault();e.stopPropagation();const id=b.dataset.fav;favs=favs.includes(id)?favs.filter(x=>x!==id):[...favs,id];b.classList.toggle('active',favs.includes(id));saveFav()})}
