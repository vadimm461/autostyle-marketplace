...
export function discountPercent(p){ const m=Number(p.discountPercent || p.discount || p.discount_percent || p.salePercent || 0), old=oldPrice(p), price=Number(p.price||0); if(m>0)return m; return old>price&&price>0?Math.round((old-price)/old*100):0; }
