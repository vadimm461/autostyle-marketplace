...
async function save() {
  cart = normalizeUserCart(cart);
  try { await saveUserCart(cart); } catch (_) { updateCartCounter(); }
}
...
// Важно: cart.html только читает корзину конкретного пользователя из autostyle_users/{uid}.cart.
// Никакой localStorage здесь не используется и пустая гостевая корзина не затирает Firebase.
onAuthStateChanged(auth, async user => {
  ...
  try {
    ...
    console.error('cart auth load error', err);
    cartList && (cartList.innerHTML = '<div class="cart-empty-card"><h2>Не удалось загрузить корзину</h2><p>Обнови страницу или проверь доступ к Firebase.</p></div>');
  } finally {
    window.AutoStyleLoader?.hide?.();
  }
});
