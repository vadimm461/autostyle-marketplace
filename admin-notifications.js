...
function loadNotificationHistoryOnce(force = false) {
  if (!$('#notifications')) return;
  if (notificationHistoryLoaded && !force) return;
...
function bind(){
  ...
  window.addEventListener('autostyle:admin-section-open', e => {
    if (e.detail?.section === 'notifications') loadNotificationHistoryOnce();
  });

  if ($('#notifications')?.classList.contains('active')) loadNotificationHistoryOnce();
}
